...
async def prepare_database():
    ...
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.drop_all) # стирает все таблицы которые наследовались от base 
        await conn.run_sync(Base.metadata.create_all) # создаст все пустые таблицы которые наследовались от base
