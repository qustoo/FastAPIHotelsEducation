...
@router.get("/byimages")
async def get_bookings_with_images(user: Users = Depends(get_current_user))-> list[SBooking]:
...
@router.get("")
async def get_bookings(user : Users = Depends(get_current_user)):# -> list[SBookingDates]:
...
async def add_booking(
    ...
    booking = await BookingDAO.add(user["Users"].id,
                                   room_id,
                                   date_from,
                                   date_to)
    if not booking:
...
async def remove_bookings(booking_id : int,
                          user : Users = Depends(get_current_user)):
